...
def upload_volunteers_page():
    st.title("อัปโหลดข้อมูลอาสาสมัคร")
    ...
    if uploaded_file is not None:
        try:
            df = pd.read_excel(uploaded_file, engine="openpyxl", dtype={"phone_number": str, "volunteer_id": str})
                ...
                if st.button("อัปโหลดข้อมูล", key="upload_btn"):
                    ...
                    try:
                        for _, row in df.iterrows():
                            ...
                            sql = f"INSERT INTO volunteers ({columns}) VALUES ({placeholders})"
